...
import security.payload.SignupRequest;
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
        if (userRepository.existsByUsername(signUpRequest.getUsername())) {
            return ResponseEntity
                    .badRequest()
                    .body(new FeastErrorResponse(HttpStatus.NOT_FOUND.value(), "Error: Username is already taken!"));
        }

        if (userRepository.existsByEmail(signUpRequest.getEmail())) {
            return ResponseEntity
                    .badRequest()
                    .body(new FeastErrorResponse(HttpStatus.NOT_FOUND.value(), "Error: Email is already in use!"));
        }
